...
"""
    DungeonDraw 2.7 - A small dungeon editor for role-playing games.
    Copyright (C) 2022, 2026 Hauke Lubenow
...
NROFLINES_Y     = 25
LINESIZE        = 32
WINDOW_GEOMETRY = "1920x950+0+0"

# 2. Fonts:
if os.name == "posix":
    # Linux:
    APPLICATIONFONT = ("Sans", 24)
    LETTERFONT_TK   = ("Arial", 24)
    LETTERFONT_PIL  = ("/usr/share/fonts/truetype/LiberationSans-Regular.ttf", 18)
    FILENAMEFONTSIZE = 24
else:
    # Windows:
...
# Depicts the order of the menu items in the GUI:
DRAWENTRIES = ("wall",
               "door",
               "door_locked",
               "stairs",
...
class Board:
    ...
    def __init__(self):
        self.border     = 20
        self.linestates = ("empty", "wall", "door", "door_locked", "transparentwall")
    ...
    def collectData(self):
        ...
        """ About the file format when using "saveAs":
            There's one line in the file for one line on the canvas.
            The line in the file begins with the index in "self.linestates":
                0 : "empty"
                1 : "wall"
                2 : "door"
                3 : "door_locked"
                4 : "transparentwall"
            If there's an attachment, a comma and one of these words are added:
                key
                stairs
                circle_red
                circle_green
                circle_blue
                letter
            If the attachment's name is 'letter', a colon and the corresponding letter is added to the line in the file.
            Example: "1,key": The line has a door and a key. """
...
class Line:
    ...
    def drawPIL(self, pildraw, pilfont):
        ...
        self.drawAttachmentPIL(pildraw, pilfont)

        if self.state == "empty":
            if DRAW_MEASURING_TAPE_WITH_PIL and self.isgrey:
                self.drawPILLine(pildraw, self.p1, self.p2, 1, COLORS["pil"]["measuring"])
            else:
                self.drawPILLine(pildraw, self.p1, self.p2, 1, COLORS["pil"]["empty"])

        if self.state == "wall":
            self.drawPILLine(pildraw, self.p1, self.p2, WALLWIDTH, COLORS["pil"]["wall"])

        if self.state in ("door", "door_locked"):
            if self.door:
        ...
        if self.state == "transparentwall":
            for p in self.lparts:
                self.drawPILLine(pildraw, p[0], p[1], 3, COLORS["pil"]["transparentwall"])

    def drawPILLine(self, pildraw, p1, p2, width, color):
        p = (p1[0], p1[1], p2[0], p2[1])
        pildraw.line(xy = p, width = width, fill = color)
    ...
    def drawAttachmentPIL(self, pildraw, pilfont):
        if not self.attachment:
            return
        if self.attachment.name == "letter":
            self.attachment.drawPIL(pildraw, pilfont) 
        else:
            self.attachment.drawPIL(pildraw, None)
...
class Door:
    ...
    def __init__(self, line, locked):
        ...
        if self.locked:
            ...
            self.color_pil = COLORS["pil"]["door_locked"]
        else:
            self.color_tk  = COLORS["tk"]["door"]
            self.color_pil = COLORS["pil"]["door"]
    ...
    def drawPIL(self, pildraw):

        oc = self.getOvalCoordinates()

        pildraw.ellipse(oc, fill = None, outline = self.color_pil)
        if self.line.orientation == "horizontal":
            self.line.drawPILLine(pildraw, self.line.p1, self.line.p2, 3, self.color_pil)
        else:
            self.line.drawPILLine(pildraw, self.line.p1, self.line.p2, 2, self.color_pil)
...
class Stairs(Attachment):
    ...
    def drawPIL(self, pildraw, pilfont):
        sc = self.getStairCoordinates()
        for i in sc:
            pildraw.line(xy = (i[0][0], i[0][1], i[1][0], i[1][1]), width = 2, fill = COLORS["pil"]["stairs"])
...
class Circle(Attachment):
    ...
    def drawPIL(self, pildraw, pilfont):
        cc = self.getCircleCoordinates()
        pildraw.ellipse((cc[0][0], cc[0][1], cc[1][0], cc[1][1]), fill = COLORS["pil"][self.name], outline = COLORS["pil"][self.name])
...
class Main:
    ...
    def __init__(self):
        ...
        # Set font-sizes of filenames in file dialogues (Linux only atm):
        if os.name == "posix":
            tkfont.nametofont("TkDefaultFont").configure(size = FILENAMEFONTSIZE)
            tkfont.nametofont("TkIconFont").configure(size = FILENAMEFONTSIZE)
        self.mw.geometry(WINDOW_GEOMETRY)
        self.defaulttitle = "DungeonDraw"
        self.windowtitle  = self.defaulttitle
        ...
        self.menu_draw.insert_separator(DRAWENTRIES.index("separator_4"))
    ...
    def getSaveName(self, caller, initdir, filetypes):
        initfile = ""
        if caller == "image" and self.windowtitle != self.defaulttitle:
            initfile = self.windowtitle
            if initfile.endswith(".map"):
                initfile = initfile[0:-4]
    ...
    def saveAs(self):
        filename = self.getSaveName("data", FILEDIR, (("map files", "*.map"),) )
        if not filename:
            return
    ...
    def saveImage(self):
        ...
        if self.lettersused:
            if os.path.exists(LETTERFONT_PIL[0]):
                ...
                # pilfont's still None.

        for line in self.board.lines:
            line.drawPIL(pildraw, pilfont)
